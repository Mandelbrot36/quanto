...
def wallet_delta2(gold_paths, usdpln_paths, T_years, params, K, option_type='call', n_hedge=252, theoretic=False):
    """
    Zakładamy że w momencie zero zaczynamy trading. Wykonanie akcji odbywa się w ostatnim kroku.
    (ostatnim dniu, bądź na zamknięcie giełdy ostatniego dnia)
    ...
    if not theoretic:
        ...
        D = params['r'] - params['r_f'] - params['usdpln']['std']**2 + params['rho_y_theoretic'] * params['Y_theoretic']['std'] * params['usdpln']['std']
        sigma_y = np.sqrt(params['Y_theoretic']['std']**2 + params['usdpln']['std']**2 - 2 * params['rho_y'] * params['Y_theoretic']['std'] * params['usdpln']['std'])


    option_value[:, 0] = option_price(gold_0/usdpln_0,
                                      K,
                                      t_to_maturity, params['r'],
                                      sigma_y,
                                      params['gold']['s0'],
    ...
    # Początkowy skład portfela
    gold_position[:, 0] = delta
    usd_position[:, 0] = (-gold_0 / usdpln_0) * gold_position[:, 0]
    #usd_value[:, 0] = usd_position[:, 0] * usdpln_0
    ...
    for i in range(1, steps - 1):
        ...
        option_value[:, i] = option_price(gold_current/usdpln_current,
                                          K,
                                          t_to_maturity, params['r'],
                                          sigma_y,
                                          params['gold']['s0'],
        ...
        new_delta_1 = option_delta(gold_current/usdpln_current,
                                   usdpln_current,
                                   K,
                                   t_to_maturity,
                                   params['r'],
    ...
    option_value[:, -1] = option_price(gold_T/usdpln_T,
                                       K,
                                       0, params['r'],
                                       sigma_y,
                                       params['gold']['s0'],
